...
###Intraspecific Variation with fossils
###Geometric Morphometric of the Humerus - Fossils included
###Vallejo-Pareja et al in prep
###############################################################################

rm(list=ls(all=T))
setwd("C:/Users/vacam/Dropbox (UFL)/Oligocene Frogs/Eleutherodactylus_paper/GMM/GMM_Intraspecific_2")
getwd()
...
coords <- arrayspecs(coords, 14, 3, 36) #here it goes: landmarks, dimensions, specimens
...
maturity2<-as.factor(classifier_intra$Distal_condyle2)
...
levels(sp)
mycols_intra <- c("#44AA99", "#AA4499", "black")
plot<-plot(PCA_intra, axis1 = 1, axis2 = 2, label = TRUE, main = "GMM Humerus", 
           pch = c(21,0, 24)[maturity2], cex = 0.6*Y.gpa_intra$Csize, col=mycols_intra[sp], 
           bg=adjustcolor(mycols_intra[sp],0.6))
legend('bottomleft', pch=21, col=mycols_intra,
       pt.bg=adjustcolor(mycols_intra, 0.6),
       paste('', levels(sp)),  cex=0.6, title='Species')
legend('bottomright', pch=c(21,24), 
       paste('', levels(maturity)),  cex=0.6, title='Humerus condyle')
...
#capture.output(test_intra, file="Intra_fossils_ANOVA_test.csv")
capture.output(test_intra, file="Intra_fossils_ANOVA_test.txt")
test_table<-as.data.frame (an_Y.gpa_intra$aov.table)
test_table$`Pr(>F)`
...
#capture.output(test_intra, file="Intra_fossils_maturity_ANOVA_test.csv")
capture.output(test_intra_maturity, file="Intra_fossils_maturity_ANOVA_test.txt")
...
#New plot
levels(new_sp)
new_mycols_intra <- c("#44AA99", "#AA4499", "black")
plot<-plot(new_PCA_intra, axis1 = 1, axis2 = 2, label = TRUE, main = "GMM Humerus of mature individuals", 
           pch = c(21,0)[new_maturity], cex = 0.6*new_Y.gpa_intra$Csize, col=new_mycols_intra[new_sp], 
...
#capture.output(test_intra, file="Intra_fossils_matureonly_ANOVA_test.csv")
capture.output(new_test_intra, file="Intra_fossils_matureonly_ANOVA_test.txt")
...
legend('top', pch=21, col=new_mycols_intra,
       pt.bg=adjustcolor(new_mycols_intra, 0.8),
       paste('', levels(new_sp)),  cex=0.8, title='Species')
